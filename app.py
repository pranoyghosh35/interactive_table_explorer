    ...
    try:
        ...
        if len(sheet_names) > 1:
            selected_name = st.selectbox(
                "Select a sheet or table", sheet_names)
        else:
        ...
        st.write(
            f"Selected {selected_name} with {df.shape[0]} rows and {df.shape[1]} columns.")
        st.write("Columns:")
        st.markdown(
            f"<div style='color:green'>{', '.join(df.columns)}</div>",
            unsafe_allow_html=True)

        # User input for display options
        display_option = st.radio(
            "Choose display option", ("Head and Tail", "Random Sample"))
        n_rows = len(df)

        if display_option == "Random Sample":
            n_random_sample = st.number_input(
            ...
            n_head = st.number_input(
                "Number of rows to display from the head",
                min_value=1,
                max_value=n_rows // 2,
                value=5)
            n_tail = st.number_input(
                "Number of rows to display from the tail",
                min_value=1,
                max_value=n_rows // 2,
                value=5)
        ...
        with tabs[0]:
            ...
            marker = st.selectbox(
                "Select marker", [
                    'circle', 'x', 'cross', 'triangle-up', 'triangle-down'])
            marker_size = st.slider(
                "Marker size", min_value=1, max_value=10, value=2)
            x_lim = st.slider(
                "X-axis limit",
                float(
                    df[x_axis].min()),
